...
function writeMsgInFile($fileFullPath, $lineToWrite, $erase)
{
    ...
    */
    ...
    $lineSplitted = explode("\t",$lineToWrite);
    if ($lineSplitted[2] != "" || count($lineSplitted) > 3) {
        $strWriter = null;
        if ($erase) {
...
function checkMsgToWrite($msg, $minLength, $maxLength)
{
    ...
    */
    ...
    $msgAfterCount = "";
    $counter = nbOfCharInMsg($msg);
    if ($counter >= $minLength) {
        if ($counter > $maxLength) {
            $msgAfterCount = substr($msg,0, $maxLength);
        }else{
            $msgAfterCount = $msg;
        }
    }else{
